...
class Roomba {
    ...
    readFromFile(pathToInputFile) {
        ...
        return new Promise((resolve, reject) => fs_1.readFile(targetFile, (e, d) => {
            if (e)
                reject(`Error importing file: ${e.message}`);
            resolve(d.toString());
        }));
    }
    // ingest input string and update instance state with results
    ingestInput(inputString) {
        // split input string on newlines
        const [dimensions, initialCoordinates, ...remainingRows] = inputString.split("\n");
